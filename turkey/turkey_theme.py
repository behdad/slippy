...
def prepare_page (renderer):
	...
	cr.set_source_rgb (227/255., 10/255., 23/255.)
	cr.paint ()

	cr.move_to (.5 * width, height-p2)
	cr.set_source_rgba (1, 1, 1)
	renderer.put_text ("Turkey, Feb 2009", height=f-p2, valign=-1)

	cr.move_to (p, p)
	renderer.put_image ("gnome-white.svg", height = l-p2, valign=+1, halign=+1)

	cr.move_to (width-p, p)
	renderer.put_image ("turkey-flag.svg", height = l-p2, valign=+1, halign=-1)
...
def draw_bubble (renderer, x, y, w, h, data=None):
	...
	bubble (cr, xc, yc, x, y, w, h)
	cr.set_source_rgb (227/255.*a, 10/255.*a, 23/255.*a)
	cr.set_line_width (p)
